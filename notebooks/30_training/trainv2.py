...
# --- 5c. LightGBM (log1p target) ---
print("💡 Training LightGBM (log1p)...")
lgbm_log = LGBMRegressor(
 n_estimators=500, max_depth=8, learning_rate=0.05,
 subsample=0.8, colsample_bytree=0.8,
 n_jobs=-1, random_state=RANDOM_STATE, verbose=-1,
)
lgbm_log.fit(X_train, y_log_train)
lgbm_log_preds = np.expm1(lgbm_log.predict(X_test))
results.append(evaluate("LightGBM (log1p)", y_reg_test, lgbm_log_preds))
...
#   LightGBM (log1p) 13.739007 5.164441 0.095257
